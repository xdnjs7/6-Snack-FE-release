    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          aria-label="챗봇 닫기"
          className="rounded-md p-1 text-[var(--color-primary-950)]/80 transition-colors hover:bg-[var(--color-primary-100)] hover:text-[var(--color-primary-950)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary-200)]"
        >
          ✕
        </button>
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-[var(--color-primary-200)]/30 to-transparent" />
      </header>
      ...
      <div className="scrollbar relative flex-1 space-y-4 overflow-y-auto px-4 py-4 text-sm leading-relaxed">
            ...
            <div key={m.id || m.createdAt} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
              <div
                className={`group relative max-w-[85%] whitespace-pre-wrap break-words rounded-lg px-3 py-2 shadow-sm ring-1 ring-black/5 ${isUser
                  ? 'bg-[var(--color-primary-200)] text-[var(--color-primary-950)]'
                  : 'bg-[var(--color-primary-50)] text-[var(--color-primary-950)] backdrop-blur-sm'
              ...
              >
                ...
                {m.error && (
                  <button
                    type="button"
                    onClick={onResend}
                    className="absolute -right-2 -top-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-[var(--color-primary-50)] text-red-500 shadow ring-1 ring-black/5 transition-colors hover:bg-red-500 hover:text-white"
                    aria-label="다시 시도"
                  >
                    ↺
                  </button>
              ...
              <span className="mt-1 select-none text-[10px] font-medium text-[var(--color-primary-400)]">
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="max-w-[70%] animate-pulse rounded-lg bg-[var(--color-primary-100)] px-3 py-2 text-[var(--color-primary-400)] ring-1 ring-black/5">
              응답 생성 중...
            </div>
          </div>
        )}
      ...
      >
        ...
        <p className="mt-2 line-clamp-2 text-[10px] text-[var(--color-primary-400)]">
