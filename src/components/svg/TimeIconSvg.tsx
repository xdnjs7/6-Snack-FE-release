import React from "react";

type TTimeIconSvgProps = {
  className?: string;
  isActive?: boolean;
};

export default function TimeIconSvg({ className = "", isActive = false }: TTimeIconSvgProps) {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={`cursor-pointer ${className}`}
    >
      <path
        d="M20.5 12C20.5 7.30558 16.6944 3.5 12 3.5C7.30558 3.5 3.5 7.30558 3.5 12C3.5 16.6944 7.30558 20.5 12 20.5V22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22V20.5C16.6944 20.5 20.5 16.6944 20.5 12Z"
        fill={isActive ? "#000000" : "currentColor"}
      />
      <path d="M11.25 6.90527H12.75V11.25H16.3281V12.75H11.25V6.90527Z" fill={isActive ? "#000000" : "currentColor"} />
    </svg>
  );
}
